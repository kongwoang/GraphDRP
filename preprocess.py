...
def is_not_float(string_list):
    """
    Purpose: Checks if any string in a list cannot be converted to a float.
    Logic: Attempts to convert each string to a float. If any conversion fails, returns True (indicating non-float values); otherwise, returns False.
    Use Case: Used to filter invalid or non-numeric compound IDs (CIDs) in cid_from_other_source.
    """
    try:
...
folder = "/kaggle/input/convnet4drp-dataset/"
#folder = ""
